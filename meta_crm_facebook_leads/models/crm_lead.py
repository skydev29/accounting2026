...
class CrmLead(models.Model):
    ...
    facebook_ad_id = fields.Many2one(
        'utm.medium', related='medium_id',
        store=True, readonly=True, string='Facebook Ad'
    )
    ...
    def prepare_lead_creation(self, lead, form):
        ...
        vals.update({
            ...
            'medium_id': (
                form.medium_id and form.medium_id.id or
                self.get_ad(lead)
            ),
            'user_id': (
                form.team_id and
    ...
    def lead_processing(self, r, form, latest_date=None):
        ...
        for lead in r['data']:
            ...
            # Track the most recent created_time across all fetched leads
        ...
        if r.get('paging') and r['paging'].get('next'):
            ...
            latest_date = self.lead_processing(
                requests.get(r['paging']['next']).json(), form, latest_date
            )
        return latest_date
    ...
    def get_facebook_leads(self):
        ...
        for form in self.env['crm.facebook.form'].search([]):
            ...
            r = requests.get(
                GRAPH_API + form.facebook_form_id + '/leads',
                params=params
            ).json()
            if r.get('error'):
                raise UserError(r['error']['message'])
            latest_date = self.lead_processing(r, form)
            # Advance the watermark so the next run only fetches new leads
            if latest_date:
