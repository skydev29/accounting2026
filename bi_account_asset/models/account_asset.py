...
class AccountAssetCategory(models.Model):
    ...
    account_depreciation_id = fields.Many2one(
        'account.account', string='Depreciation Entries: Asset Account', required=True,
        domain=[('account_type', '=', 'income_other')],
        help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one(
        'account.account', string='Depreciation Entries: Expense Account', required=True,
        domain=[('account_type', '=', 'income_other')],
...
class AccountAssetAsset(models.Model):
    ...
    state = fields.Selection(
        ...
        help="When an asset is created, the status is 'Draft'.\n"
             ...
             "You can manually close an asset when the depreciation is over. If the last line of "
             "depreciation is posted, the asset automatically goes in that status.")
    active = fields.Boolean(default=True)
    ...
    prorata = fields.Boolean(
        string='Prorata Temporis',
        help='Indicates that the first depreciation entry for this asset have to be done '
             'from the purchase date instead of the first January / Start date of fiscal year')
    depreciation_line_ids = fields.One2many(
        'account.asset.depreciation.line', 'asset_id',
        string='Depreciation Lines')
    salvage_value = fields.Float(
        string='Salvage Value', digits=0,
    ...
    def _get_last_depreciation_date(self):
        self.env.cr.execute("""
            SELECT a.id as id, COALESCE(MAX(m.date), a.asset_date) AS date
        ...
        if self.method_time == 'end':
            ...
            while depreciation_date <= end_date:
                depreciation_date = date(
                    depreciation_date.year, depreciation_date.month, depreciation_date.day
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata:
                if posted_depreciation_line_ids and posted_depreciation_line_ids[-1].depreciation_date:
                    last_depreciation_date = fields.Date.to_date(
                ...
                asset_dt = fields.Date.to_date(self.asset_date) or fields.Date.context_today(self)
                if self.method_period >= 12:
                    asset_date = date(asset_dt.year, 1, 1)
                else:
                    asset_date = date(asset_dt.year, asset_dt.month, 1)
    ...
    def validate(self):
        ...
        field_names = [
            ...
            'method_progress_factor', 'method_time', 'salvage_value', 'invoice_id',
        ]
        ref_tracked_fields = self.env['account.asset.asset'].fields_get(field_names)
        for asset in self:
            ...
            dummy, tracking_value_ids = asset._mail_track(tracked_fields, dict.fromkeys(field_names))
    ...
    def copy_data(self, default=None):
        ...
        for vals in vals_list:
            vals['name'] = vals.get('name', '') + _(' (copy)')
        return vals_list
...
class AccountAssetDepreciationLine(models.Model):
    ...
    move_check = fields.Boolean(
        compute='_get_move_check', string='Linked', tracking=True, store=True)
    move_posted_check = fields.Boolean(
        compute='_get_move_posted_check', string='Posted', tracking=True, store=True)
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            if line.move_id:
                raise UserError(
                    _('This depreciation is already linked to a journal entry! '
            ...
            amount = current_currency._convert(
                line.amount, company_currency,
                line.asset_id.company_id, depreciation_date)
            ...
            move_line_1 = {
                ...
                'analytic_distribution': (
                    category_id.analytic_distribution if category_id.type == 'sale' else False),
                'currency_id': current_currency.id if current_currency else False,
                'date_maturity': depreciation_date or False,
            }
            move_line_2 = {
                'name': asset_name,
                ...
                'partner_id': line.asset_id.partner_id.id,
                'analytic_distribution': (
                    category_id.analytic_distribution if category_id.type == 'purchase' else False),
                'currency_id': current_currency.id if current_currency else False,
                'date_maturity': depreciation_date or False,
            }
            move_vals = {
        ...
        if post_move and created_moves:
            created_moves.filtered(
                lambda m: any(
                    m.asset_depreciation_ids.mapped('asset_id.category_id.open_asset'))
            ).action_post()
    ...
    def create_grouped_move(self, post_move=True):
        ...
        depreciation_date = (
            self.env.context.get('depreciation_date') or fields.Date.context_today(self))
        amount = 0.0
        for line in self:
            company_currency = line.asset_id.company_id.currency_id
            current_currency = line.asset_id.currency_id
