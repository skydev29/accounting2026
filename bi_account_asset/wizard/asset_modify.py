...
class AssetModify(models.TransientModel):
    ...
    def default_get(self, field_names):
        res = super(AssetModify, self).default_get(field_names)
        asset_id = self.env.context.get('active_id')
        asset = self.env['account.asset.asset'].browse(asset_id)
        if 'name' in field_names:
            res.update({'name': asset.name})
        if 'method_number' in field_names and asset.method_time == 'number':
            res.update({'method_number': asset.method_number})
        if 'method_period' in field_names:
            res.update({'method_period': asset.method_period})
        if 'method_end' in field_names and asset.method_time == 'end':
    ...
    def modify(self):
        ...
        if changes:
            asset.message_post(
                subject=_('Depreciation board modified'),
                body=self.name,
